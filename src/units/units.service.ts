...
export class UnitsService {
    ...
    async listUnits(getUnitsFilterDto: GetUnitsFilterDto, user: User): Promise<Unit[]> {
        return await this.unitsRepository.getUnits(getUnitsFilterDto, user);
    }
    ...
    async createUnit(createUnitDto: CreateUnitDto, user: User): Promise<Unit> {
        ...
        if (createUnitDto.parent) {
            parent = await this.unitsRepository.findOne({where: {id: createUnitDto.parent}});
        ...
        const nameExists = await this.unitsRepository.findOne({where: {name: createUnitDto.name}});
    ...
    async isManagerInTree(unitId: number, user: User): Promise<boolean> {
        ...
        const unitsTreeParentObject = await this.unitsRepository.getAncestors(unitId);
    ...
    async updateUnit(id: number, updateUnitDto: UpdateUnitDto, user: User): Promise<Unit> {
    ...
    async haveUnitUser(unitId: number): Promise<boolean> {
        ...
        const have = !!(await Promise.all([...descendants.map(d => this.usersService.getUsers({unitId: d.id}))]))
            .map(d => d.length)?.some(d => d > 0);
        return have;
    }
