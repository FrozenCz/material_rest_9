...
import {CreateHistory, HistoryDto, HistoryRelatedTo} from './models/history.model';
...
export class HistoryService {
    ...
    private static compareValueDifferences(obj1, obj2): { changedFrom: any, changedTo: any } {
    ...
    async saveHistory(createHistory: CreateHistory, user: User): Promise<History> {
        ...
        switch (createHistory.relatedTo) {
            ...
            case HistoryRelatedTo.assetsChangeInformation:
                history.asset = createHistory.asset;
                const compared = HistoryService.compareValueDifferences(createHistory.changedFrom, createHistory.changedTo);
            ...
            case HistoryRelatedTo.assetsUserChange:
                ...
                history.changedTo = JSON.stringify({userId: userTo.id, name: userTo.name, surname: userTo.surname});
                break;
    ...
    async getHistoryForAsset(assetId: number): Promise<HistoryDto[]> {
        return (await this.historyRepository.find({where: {asset: {id: assetId}},order: {id: 'DESC'}})).map(history => this.processHistory(history));
    }
    ...
    private processHistory(history: History): HistoryDto {
        const historyDto: HistoryDto = {
            id: history.id,
            changedBy: {
                id: history.changedBy.id, name: history.changedBy.name, surname: history.changedBy.surname,
                unit: {id: history.changedBy.unit.id, name: history.changedBy.unit.name}
            },
        ...
        if (history.user) {
            historyDto.user = {
                id: history.user.id, name: history.user.name, surname: history.user.surname, unit: {
                    id: history.user.unit.id, name: history.user.unit.name
                }
            }
        }
        return historyDto;
