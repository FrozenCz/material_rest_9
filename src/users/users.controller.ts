...
export class UsersController {
    ...
    getUsers(@GetUser() user: User, @Query(ValidationPipe) getUsersFilterDto: GetUsersFilterDto
        ...
        return this.api.getUsers(getUsersFilterDto);
    }
    ...
    updateUser(@Param('id', ParseIntPipe) id:number, @GetUser() user:User,
    ...
    @UseGuards(AuthGuard(), RightsGuard)
    @RightsAllowed(RightsTag.updateUsersInformation)
    updateUsersUnits(@GetUser() user: User, @Body(ValidationPipe) updateUsersDto: UpdateUsersDto): Promise<User>[] | any {
    ...
    setUsersRights(@Param('id', ParseIntPipe) userId: number, @GetUser() user: User, @Body(ValidationPipe) setUserRightsDto: SetUserRightsDto): Promise<Rights[]> {
        if ((!setUserRightsDto.removeRights && !setUserRightsDto.addRights) || (setUserRightsDto.removeRights?.length===0 && setUserRightsDto.addRights?.length === 0)) throw new BadRequestException('You must specified changes');
