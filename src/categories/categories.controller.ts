...
export class CategoriesController {
    ...
    getCategoriesSettings(@Param('typeOfSettings') typeOfSettings: CategorySettingsEnum) {
        if (!Object.values(CategorySettingsEnum).includes(typeOfSettings)) {
            return new BadRequestException('typeOfSettings must be one of '+CategorySettingsEnum)
        }
    ...
    @UseGuards(AuthGuard(), RightsGuard)
    @RightsAllowed(RightsTag.createCategory)
    updateSettings(@Body(ValidationPipe) updateCategorySettingsDto: UpdateCategorySettingsDto, @GetUser() user: User): Promise<void> {
    ...
    @UseGuards(AuthGuard(), RightsGuard)
    @RightsAllowed(RightsTag.createCategory)
    updateCategory(@Param('id', ParseIntPipe) id: number, @Body(ValidationPipe) updateCategoryDto: UpdateCategoryDto, @GetUser() user: User): Promise<Category> {
        return this.categoriesService.updateCategory({id, ...updateCategoryDto}, user);
    }

    @Get('/:idCategory/descendants')
    getDescendantsById(@Param('idCategory', ParseIntPipe) idCategory: number): Promise<Category[]> {
        return this.categoriesService.getDescendantsById(idCategory);
    }

    @Get('/:idCategory/isAbleToDelete')
    @UseGuards(AuthGuard())
    isAbleToDelete(@Param('idCategory', ParseIntPipe) idCategory: number): Promise<boolean> {
