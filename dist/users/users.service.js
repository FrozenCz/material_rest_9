...
let UsersService = class UsersService {
    ...
    async getUsers(getUsersFilterDto) {
        ...
        if (unitId) {
            return user_entity_1.User.find({ where: { active: true, unit: { id: unitId } } });
        }
        else if (unitIds) {
            return user_entity_1.User.find({ where: { active: true, unit: { id: (0, typeorm_2.In)(unitIds) } } });
        }
        else {
            return user_entity_1.User.find({ where: { active: true } });
        }
    }
    ...
    async getUsersByRightTag(tag) {
        return this.userRepository.find({
            where: {
                rights: {
                    tag,
                },
                unit: { id: (0, typeorm_2.Not)((0, typeorm_2.IsNull)()) },
            },
        });
    }
};
