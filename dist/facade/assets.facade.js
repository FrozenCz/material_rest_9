...
const stock_taking_item_entity_1 = require("../assets/models/stock-taking-item.entity");
...
let AssetsFacade = class AssetsFacade {
    ...
    async getItems(stockTaking, assetsMap) {
        return await Promise.all(stockTaking.items.map(async (item) => {
            ...
            return Object.assign(Object.assign({}, item), { id: item.assetId, assetName: found.name, serialNumber: found.serialNumber, location: await found.location });
        }));
    }
    ...
    async patchStockTakingInProgress(param) {
        ...
        const assetsMap = await this.assetsService.getAssetsMap$();
        const locationsMap = utilFuncs_1.UtilFuncs.createMap({
            propertyName: 'uuid',
    ...
    updateStockTakingItems(param) {
        ...
        stock_taking_item_entity_1.StockTakingItemEntity.save(stockTakingItems);
