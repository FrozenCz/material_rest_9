...
let UsersFacade = class UsersFacade {
        ...
        return this.usersService.getUsers(getUsersFilterDto).then((users) => {
            return users.map((u) => {
                ...
                return {
                    ...
                    unit_id: (_a = u.unit) === null || _a === void 0 ? void 0 : _a.id,
                };
            });
        });
    }
    ...
    getCaretakers() {
        return this.usersService
            .getUsersByRightTag(rights_list_1.RightsTag.createAssets)
            ...
            return {
                ...
                unit_name: (_a = user.unit) === null || _a === void 0 ? void 0 : _a.name,
                unit_id: (_b = user.unit) === null || _b === void 0 ? void 0 : _b.id,
            };
        }));
    }
    async createRequestForAssetTransfer(requestForAssetTransfer) {
        const { caretakerFrom, caretakerTo } = await this.transformIdToCaretakers(requestForAssetTransfer);
        const assets = await this.assetsService.getAssets(requestForAssetTransfer.assetIds);
    ...
    async transformIdToCaretakers(requestForAssetTransfer) {
        const caretakers = await this.getCaretakers();
        const caretakerFrom = caretakers.find((caretaker) => caretaker.id === requestForAssetTransfer.fromUser);
        const caretakerTo = caretakers.find((caretaker) => caretaker.id === requestForAssetTransfer.toUser);
        if (!caretakerFrom || !caretakerTo) {
            throw new common_1.NotFoundException('Caretaker not found!');
        }
